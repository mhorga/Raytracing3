...
public func imageFromPixels(width: Int, _ height: Int) -> CIImage {
    ...
    var object = sphere(c: float3(x: 0, y: -100.5, z: -1), r: 100, m: metal(a: float3(x: 0.8, y: 0.3, z: 0.3)))
    world.add(object)
    object = sphere(c: float3(x: 0, y: 0, z: -1), r: 0.5, m: metal(a: float3(x: 0.8, y: 0.8, z: 0)))
    world.add(object)
    let cam = camera()
    for i in 0..<width {
        for j in 0..<height {
